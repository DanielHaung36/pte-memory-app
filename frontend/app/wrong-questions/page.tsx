...
const ERROR_TYPES = {
  understanding: { label: "理解错误", icon: <Brain className="h-4 w-4" />, color: "text-red-600 bg-red-100" },
  carelessness: { label: "粗心大意", icon: <AlertTriangle className="h-4 w-4" />, color: "text-orange-600 bg-orange-100" },
  knowledge_gap: { label: "知识盲区", icon: <Eye className="h-4 w-4" />, color: "text-purple-600 bg-purple-100" },
  time_pressure: { label: "时间压力", icon: <Clock className="h-4 w-4" />, color: "text-blue-600 bg-blue-100" },
  grammar: { label: "语法错误", icon: <PenTool className="h-4 w-4" />, color: "text-indigo-600 bg-indigo-100" },
  vocabulary: { label: "词汇错误", icon: <Brain className="h-4 w-4" />, color: "text-pink-600 bg-pink-100" },
  comprehension: { label: "理解错误", icon: <Eye className="h-4 w-4" />, color: "text-green-600 bg-green-100" },
  pronunciation: { label: "发音错误", icon: <Mic className="h-4 w-4" />, color: "text-yellow-600 bg-yellow-100" },
};
...
export default function WrongQuestionsPage() {
  ...
  const [selectedResolutionStatus, setSelectedResolutionStatus] = useState<string>("all");
  ...
  const { 
    ...
    is_resolved: selectedResolutionStatus === "all" ? undefined : selectedResolutionStatus === "resolved"
  });
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedErrorType, selectedResolutionStatus, sortBy, sortOrder]);
  ...
  const filteredQuestions = wrongQuestions
    .filter(question => {
      const matchesSearch = !searchTerm || 
        question.error_reason.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      const matchesErrorType = selectedErrorType === "all" || question.error_type === selectedErrorType;
      const matchesResolution = selectedResolutionStatus === "all" || 
  ...
  const getPriorityStars = (level: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <Star 
        key={i} 
        className={`h-3 w-3 ${i < Math.floor(level/2) ? 'text-red-400 fill-current' : 'text-gray-300'}`} 
      />
    ));
  };
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  >
                    {stats.resolved > 0 ? Math.round((stats.resolved / stats.total_wrong) * 100) : 0}%
                  </motion.p>
                </div>
                <motion.div 
          ...
          <AnimatePresence mode="popLayout">
            {paginatedQuestions.length === 0 ? (
              ...
              >
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">暂无错题记录</h3>
                <p className="text-gray-600 mb-6">开始学习，建立你的错题库吧！</p>
                <Link href="/questions">
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-3">
                          <motion.span 
                            ...
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                              ERROR_TYPES[question.error_type as keyof typeof ERROR_TYPES]?.color || "text-gray-600 bg-gray-100"
                            }`}
                          >
                            {ERROR_TYPES[question.error_type as keyof typeof ERROR_TYPES]?.icon}
                            <span className="ml-1">
                              {ERROR_TYPES[question.error_type as keyof typeof ERROR_TYPES]?.label || question.error_type}
                            </span>
                          </motion.span>
                          ...
                          >
                            {question.is_resolved ? (
                              ...
                              <><XCircle className="h-3 w-3 mr-1" />待解决</>
                            )}
                          </motion.span>
                          
                          <motion.span 
                            whileHover={{ scale: 1.05 }}
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getPriorityColor(question.priority)}`}
                          >
                        ...
                        <div className="space-y-3">
                          <div>
                            <p className="text-sm text-gray-600 mb-1">错误原因:</p>
                            <p className="text-gray-900 font-medium">{question.error_reason}</p>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="text-red-600 bg-red-50 rounded-lg p-3 font-mono text-sm flex items-center justify-between">
                                ...
                                <TTSButton text={question.user_answer} size="sm" variant="ghost" />
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="text-green-600 bg-green-50 rounded-lg p-3 font-mono text-sm flex items-center justify-between">
                                ...
                                <TTSButton text={question.correct_answer} size="sm" variant="ghost" />
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-gray-500">
                        创建于 {formatDate(question.created_at)} · 下次复习 {formatDate(question.last_wrong_at)}
                      </div>
                          ...
                          <AnimatePresence>
                            {dropdownOpenId === question.id && (
                              ...
                              >
                                ...
                                <button
                                  onClick={() => {
                                    handleToggleResolution(question.id, question.is_resolved);
                                    setDropdownOpenId(null);
                                  }}
                                  className="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors"
                                >
